...
export const getPlayedGames = async (options: {
  ...
  return await Promise.all([
    await Game.find(options.playedGamesQuery)
      ...
      .limit(options.limit)
      .select(
        ' _id createdAt rewardAmount status winnerPublicKeyBase58 turnCount codeBreaker codeMaster settlementTransactionHash penalizationTransactionHash finalTransactionTimestamp cancelTransactionHash'
      )
      .lean(),
